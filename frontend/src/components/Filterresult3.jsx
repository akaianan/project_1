...
const Filterresult3 = ({ filterVisible3, setFilterVisible3 }) => {
  const email = localStorage.getItem('email')
  const room1 = localStorage.getItem('room1')
  const room2 = localStorage.getItem('room2')
  ...
  const getFilter22 = async () => {
    ...
    for (const i of allId2) {
      ...
      if (data.error) {
        ...
        if (parseInt(data.listing.metadata[0]) >= parseInt(room1) && parseInt(data.listing.metadata[0]) <= parseInt(room2)) {
          console.log(data)
          allList.push([data, i])
        }
      }
